...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.split('_')[1] == '-1' or query.data.split('_')[1] == '[]':
        await context.bot.send_message(
            chat_id=chat_id,
            text="متاسفانه جزوه یا رفرنس این مبحث هنوز داخل ربات قرار نگرفته\n\nاگر درس یا مبحثی کلا توی ربات نیست و به جزوه یا رفرنس اش نیاز داری یا اینکه جزوه خوبی داری یا رفرنسی داری که داخل ربات نیست و میخوای به اشتراک بذاری که داخل ربات قرار بگیره، به من پیام بده ❤\n@MehrshadChKh",
        )
        return
    ...
    if query.data.split('_')[0] == "pamphlets":
        reply_markup = pamphlets_keyboard_creator(query.data.split('_')[1])
...
app = ApplicationBuilder().token(BOT_TOKEN).base_url().build()
